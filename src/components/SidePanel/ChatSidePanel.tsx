...
import { ChevronUp, ChevronDown, User2, Building2, Users, LayoutDashboard, Brain, MessageSquare, FolderOpen, Layers, FileText, Wrench, Sparkles, Settings, Building, Briefcase, BarChart3, Stethoscope, Compass, Network, Palette, Package, Megaphone, HelpCircle, Plus, MoreHorizontal, Search, Loader2, Move } from "lucide-react";
...
export default function ChatSidePanel() {
    ...
    useEffect(() => {
        // Don't validate while workspaces are still loading
        if (workspacesLoading || !workspaces) return;
        
        // Only clear if we have workspaces loaded and the ID doesn't exist
        if (currentBrainSpaceId && workspaceIds.size > 0 && !workspaceIds.has(currentBrainSpaceId)) {
            setCurrentBrainSpaceId(null);
        }
    }, [currentBrainSpaceId, workspaceIds, workspacesLoading, workspaces, setCurrentBrainSpaceId]);

    // Set default brain space on chat pages if none is selected or if selected one is invalid
    useEffect(() => {
        // Only set default on chat pages
        if (!pathname.startsWith("/chat")) return;
        
        // If workspaces are still loading, wait
        if (workspacesLoading || !workspaces || workspaces.length === 0) return;
        
        // If no brain space is selected OR the selected one doesn't exist, select the first one
        const isValidSelection = currentBrainSpaceId && workspaceIds.has(currentBrainSpaceId);
        if (!isValidSelection && workspaces.length > 0) {
            setCurrentBrainSpaceId(workspaces[0].id);
        }
    }, [pathname, workspacesLoading, workspaces, currentBrainSpaceId, workspaceIds, setCurrentBrainSpaceId]);
    ...
    const handleBrainSpaceChange = (value: string) => {
        ...
        setCurrentBrainSpaceId(workspaceId);
        // Only navigate if we're not on the chat page
                    ...
                    <SidebarGroup>
                        <SidebarGroupContent>
                            <div className="px-2 py-2">
                                ...
                                <Select
                                    key={`brain-space-select-${currentBrainSpaceId}-${workspaces?.length || 0}`}
                                    value={currentBrainSpaceId ? String(currentBrainSpaceId) : ""}
